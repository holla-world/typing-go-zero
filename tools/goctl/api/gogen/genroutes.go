...
const (
	defaultTokenAuthMiddleware = "TokenAuthMiddleware"
...
rest.WithPrefix("%s"),`, g.prefix)
		...
		if g.AuthEnabled {
			genOneMiddleware(dir, cfg, g.AuthMiddleware)
		}
	...
	for _, g := range api.Service.Groups {
		...
		if auth != "false" { // 默认开启token验证
			...
			if len(auth) == 0 || auth == "true" {
				...
				groupedRoutes.AuthMiddleware = auth
			}
			groupedRoutes.middlewares = append(groupedRoutes.middlewares, groupedRoutes.AuthMiddleware)
		}
